...
| 고령인구 | **실제** 서울 열린데이터광장 생활인구(행정동별) | `api/elderly.js`, `src/data/elderly.js`, `src/data/dong.js` |
...
- 생활인구 기준 비율은 등록인구보다 낮게 나온다(종로구 평균 15.2%, 강북 19.1%, 강남 12.1%).
  그래서 점수 상한을 30% → 20%로 조정했다.

### 좌표를 행정동으로 판정하는 방법
API는 행정동 코드(8자리)로 데이터를 주는데, 역지오코딩으로 얻는 동 이름은 **법정동**이라
그대로 맞출 수 없다(창신동 ↔ 창신1·2·3동). 그래서 행정동 경계 폴리곤을 싣고 점-다각형
판정으로 코드를 직접 구한다.

- 경계: [vuski/admdongkor](https://github.com/vuski/admdongkor) 2026-07-01판, 서울 427개동을 약 11m 오차로 단순화
- `public/seoul-dong.json` 199KB(gzip 45KB). 번들이 아니라 정적 파일이라 **첫 화면 로딩에
  영향이 없고**, 분석을 시작할 때 한 번만 받는다
- 코드 대응: `ADSTRD_CODE_SE` == `adm_cd2` 앞 8자리
- 판정 검증: 경복궁→청운효자동, 낙산성곽서1길→이화동, 서울역→회현동
- 경계 밖 좌표는 자치구 집계값으로 내려가고, 화면에 어느 단위인지 표시한다

같은 자치구 안에서도 값이 달라진다(창신동 분석 실측: 17.6% / 16.2% / 15.2% / 12.6%).
